...
const walletService = {
  ...
  createWallet: async function() {
    try {
      ...
      try {
        ...
        } else if (seedResponse.data && seedResponse.data.success && seedResponse.data.seedPhrase) {
        ...
        try {
          ...
          } else if (fallbackSeedResponse.data && fallbackSeedResponse.data.success && fallbackSeedResponse.data.seedPhrase) {
            // Fallback for older API format
  ...
  sendTransaction: async function(fromAddress, toAddress, amount, fee) {
    try {
      ...
      try {
        const fallbackResponse = await axios.post(`${FALLBACK_API_URL}/transaction`, {
          sender: fromAddress,
          recipient: toAddress,
          amount,
          fee
