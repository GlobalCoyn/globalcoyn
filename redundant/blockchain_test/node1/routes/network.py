...
def receive_block():
    ...
    try:
        ...
        if 'difficulty_bits' in block_data:
            ...
            # Fallback to old format - use default difficulty and try to convert difficulty_target to bits
            difficulty_bits = block_data.get('difficulty_target', 0x2100ffff)  # Use target as bits if provided
            block = Block(
                index=block_data.get('index'),
                previous_hash=block_data.get('previous_hash'),
                timestamp=block_data.get('timestamp'),
        ...
        if can_add:
            ...
            try:
                ...
                # Convert hash to int and check against target threshold
                hash_int = int(block.hash, 16)
                
                # Get target from blockchain
                if hasattr(GCN.blockchain, 'target'):
                    target = GCN.blockchain.target
                    if hash_int > target:
                        print(f"Block doesn't meet difficulty target: {block.hash} > {hex(target)}")
                        valid_block = False
                else:
                    # Fallback simple check - look for leading zeros
                    leading_zeros = len(block.hash) - len(block.hash.lstrip('0'))
                    if leading_zeros < 2:  # Require at least 2 leading zeros for basic validation
                        print(f"Block doesn't have enough leading zeros: {block.hash}")
                        valid_block = False
...
def replace_chain():
    ...
    try:
        ...
        # Update difficulty bits and target to match the new chain
        if hasattr(submitted_chain[-1], 'difficulty_bits'):
            GCN.blockchain.bits = submitted_chain[-1].difficulty_bits
            # Recalculate target from bits
            if hasattr(GCN.blockchain, 'bits_to_target'):
                GCN.blockchain.target = GCN.blockchain.bits_to_target(GCN.blockchain.bits)
